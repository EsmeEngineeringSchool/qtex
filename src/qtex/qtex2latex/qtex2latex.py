...
def matching_to_latex(info,outfile,corrige=True):
    outfile.write(macro("question",info["Q"])+newline())
    outfile.write("\\indent\\resizebox{\linewidth}{!}{%"+newline(""))
    outfile.write(benv("tikzpicture")+newline(""))
    ...
    for k,(subq,answ_text) in enumerate(zip(info['SUB_Q'],shuff_answ_text)) :
        outfile.write(tikz_matching(k,subq,answ_text,gw=maxwidth_sub_q,dw=maxwidth_answ_text)+newline(""))
    if corrige:
        outfile.write(tikz_relie(indices)+newline(""))
    outfile.write(eenv("tikzpicture")+newline(""))
